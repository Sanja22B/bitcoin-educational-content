...
cilj: Uronite u kriptografske principe koji pokreću Bitcoin novčanike.
objectives: 
...
Otkrijte tajne determinističkih i hijerarhijskih Bitcoin novčanika uz naš CYP201 kurs! Bilo da ste redovan korisnik ili entuzijasta koji želi produbiti svoje znanje, ovaj kurs nudi potpuno uranjanje u funkcionisanje ovih alata koje svakodnevno koristimo.


Saznajte o mehanizmima funkcija Hash, digitalnim potpisima (ECDSA i Schnorr), frazama Mnemonic, kriptografskim ključevima i kreiranju adresa za primanje, sve dok istražujete napredne strategije bezbednosti.


Ova obuka će vas ne samo opremiti znanjem za razumevanje strukture Bitcoin Wallet, već će vas i pripremiti da dublje zaronite u uzbudljivi svet kriptografije.


Uz jasnu pedagogiju, preko 60 objašnjavajućih dijagrama i konkretne primere, CYP201 će vam omogućiti da razumete od A do Š kako vaš Wallet funkcioniše, tako da možete sa sigurnošću navigirati univerzumom Bitcoin. Preuzmite kontrolu nad svojim UTXO-ima danas razumevanjem kako HD novčanici funkcionišu!
...
Pre nego što se upustimo u detalje konstrukcije i rada Bitcoin novčanika, počećemo sa nekoliko poglavlja o kriptografskim primitivama koje treba poznavati za ono što sledi.

Počećemo sa kriptografskim funkcijama Hash, koje su fundamentalne kako za novčanike, tako i za sam Bitcoin protokol. Otkrićete njihove glavne karakteristike, specifične funkcije korišćene u Bitcoin, a u tehnički detaljnijem poglavlju, naučićete detalje o funkcionisanju kraljice Hash funkcija: SHA256.
...
Kada dobro razumemo ove Elements kriptografije, konačno ćemo preći na srž obuke: determinističke i hijerarhijske novčanike! Prvo, postoji odeljak posvećen Mnemonic frazama, ovim sekvencama od 12 ili 24 reči koje vam omogućavaju da kreirate i obnovite svoje novčanike. Otkrićete kako se ove reči generišu iz izvora entropije i kako olakšavaju korišćenje Bitcoin.


![CYP201](assets/fr/040.webp)


Obuka će se nastaviti proučavanjem BIP39 passphrase, seed (ne treba ga mešati sa frazom Mnemonic), master lanca koda i master ključa. Detaljno ćemo videti šta su ovi Elements, njihove odgovarajuće uloge i kako se izračunavaju.
...
Heširanje je proces koji transformiše informacije proizvoljne dužine u drugi deo informacija fiksne dužine putem kriptografske Hash funkcije. Drugim rečima, Hash funkcija uzima ulaz bilo koje veličine i pretvara ga u otisak fiksne veličine, nazvan "Hash".

Hash se takođe ponekad može nazivati "digest", "kondenzat", "kondenzovano" ili "hashed".
...
- Otpornost na neovlašćeno menjanje (lavinski efekat)
...
Nepovratnost znači da je lako izračunati Hash iz ulaznih informacija, ali obrnuti proračun, to jest, pronalaženje ulaza iz Hash, je praktično nemoguć. Ovo svojstvo čini Hash funkcije savršenim za kreiranje jedinstvenih digitalnih otisaka prstiju bez ugrožavanja originalnih informacija. Ova karakteristika se često naziva funkcijom u jednom smeru.
...
Ovo svojstvo osigurava da se čak i najmanja promena originalne poruke odmah otkrije, jer ne menja samo mali deo Hash, već ceo Hash. Ovo može biti od interesa u raznim oblastima za verifikaciju integriteta poruka, softvera ili čak Bitcoin transakcija.


#### 3. Otpornost na sudar
...
U stvarnosti, matematički je neizbežno da sudari postoje za Hash funkcije, jer veličina ulaza može biti veća od veličine izlaza. Ovo je poznato kao Dirihleov princip fioka: ako je $n$ objekata raspoređeno u $m$ fioka, sa $m < n$, onda će najmanje jedna fioka nužno sadržati dva ili više objekata. Za Hash funkciju, ovaj princip se primenjuje jer je broj mogućih poruka (gotovo) beskonačan, dok je broj mogućih heševa konačan ($2^{256}$ u slučaju SHA256).
...
Za Hash funkciju od $n$ bita, otpornost na koliziju je reda $2^{\frac{n}{2}}$, u skladu sa napadom rođendana. Na primer, za SHA256 ($n = 256$), složenost pronalaženja kolizije je reda $2^{128}$ pokušaja. U praktičnim terminima, to znači da ako se kroz funkciju prođe $2^{128}$ različitih poruka, verovatno će se pronaći kolizija.


#### 4. Otpornost na drugi preimage


Otpornost na drugi preimage je još jedna važna karakteristika Hash funkcija. Ona navodi da je, za datu poruku $m_1$ i njen Hash $h$, računarski neizvodljivo pronaći drugu poruku $m_2 \neq m_1$ takvu da:
...
Dakle, otpornost na drugi preimage je donekle slična otpornosti na koliziju, osim što je ovde napad teži jer napadač ne može slobodno izabrati $m_1$.
...
Ova funkcija se koristi u mnogim aspektima Bitcoin. Na nivou protokola, uključena je u Proof-of-Work mehanizam, gde se primenjuje u dvostrukom heširanju za pretragu delimičnog sudara između zaglavlja kandidatskog bloka, kreiranog od strane Miner, i cilja težine. Ako se ovaj delimični sudar pronađe, kandidatski blok postaje važeći i može biti dodat u Blockchain.
...
Shema desno pomeranje može se videti ovako:
...
Pre nego što se izračuna HMAC, neophodno je izjednačiti ključ i konstante prema veličini bloka $B$. Na primer, ako je ključ $K$ kraći od 128 bajtova, dopunjava se nulama da dostigne veličinu $B$. Ako je $K$ duži od 128 bajtova, kompresuje se koristeći SHA512, a zatim se dodaju nule dok ne dostigne 128 bajtova. Na ovaj način se dobija izjednačen ključ nazvan $K'$. Vrednosti $\text{opad}$ i $\text{ipad}$ se dobijaju ponavljanjem njihovog osnovnog bajta ($0x5c$ za $\text{opad}$, $0x36$ za $\text{ipad}$) dok se ne dostigne veličina $B$. Tako, sa $B = 128$ bajtova, imamo:
...
Matematika eliptičkih krivih nad konačnim poljima je analogna onoj nad poljem realnih brojeva, s prilagodbom da se sve operacije izvode modulo $p$. Da bismo pojednostavili objašnjenja, u narednim poglavljima ćemo nastaviti ilustrirati pojmove koristeći krivu definisanu nad realnim brojevima, imajući na umu da je u praksi kriva definisana nad konačnim poljem.
...
- $G$ je generatorna tačka eliptičke krive koju koriste svi učesnici Bitcoin mreže;
...
Verifikacija Schnorr potpisa je jednostavnija od verifikacije ECDSA potpisa. Ovde su koraci za verifikaciju potpisa $(R_x, s)$ sa javnim ključem $K_x$ i porukom $m$.
...
Kao što smo videli u prethodnim poglavljima, očvrsli ključevi indeksa počinju od $2^{31}$, ili `0x80000000` u heksadecimalnom obliku. Stoga, kada indeks prati apostrof u putanji derivacije, $2^{31}$ mora biti dodato naznačenom broju da bi se dobila stvarna vrednost koja se koristi u HMAC-SHA512 funkciji. Na primer, ako putanja derivacije specificira $/44'/$, stvarni indeks će biti:
...
Opcodi korišćeni u skripti su dizajnirani da manipulišu informacijama i, ako je potrebno, da ih uporede ili testiraju. Uzmimo primer P2PKH skripte, koja je sledeća:
...
- Izdvajanje 3 najznačajnija bita i 5 najmanje značajnih bitova:
  ...
  - 3 najznačajnija bita: `011` (3 u decimalnom)
  - 5 najmanje značajnih bitova: `00010` (2 u decimalnom)
